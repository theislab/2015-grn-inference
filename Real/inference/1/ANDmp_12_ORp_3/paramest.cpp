...
int main(int argc, char** argv) {
	...
	if(argc < 6)
	{
		std::cerr << "input <temperature> <number of temperatures> <gene index> <emulator-1> <emulator-2> <emulator-3> <Ob>\n";
       	return EXIT_FAILURE;
    }
    ...
    const int    emu2Ind      = atof(argv[5]);
    const int    emu3Ind      = atof(argv[6]);
    const int    Ob1    	  = atof(argv[7]);         // number observations
    ...
    const double ngenes = 4;
    const int 	 nPar   = 9;          // number parameters
    ...
    inFile.open("/home/icb/andrea.ocone/Real/inference/DataBranch_1.txt");
    for(int i = 0; i < Ob1; i++){
    ...
    inFile.open("/home/icb/andrea.ocone/Real/inference/TimesBranch_1.txt");
    for(int i = 0; i < Ob1; i++){
        inFile >> times1[i];
    }
    for(int i = 0; i < Ob1; i++){
    ...
    inFile.open("/home/icb/andrea.ocone/Real/inference/InterpolatorsCLP.txt");
    for(int i = 0; i < N; i++){
    ...
    for(int i = 0; i < N; i++){
    	emul3A[i][0] = emulTotA[i][emu3Ind-1];
	}
    ...
    // uniform prior
    double priorMax[nPar] = {log(2000), log(2000), log(50), log(2000), log(30), log(2000), log(30), log(2000), log(30)};
	...
	std::vector<double>                sLik(T,0);    
 	std::string                        fileName = "/home/icb/andrea.ocone/Real/inference/1/ANDmp_12_ORp_3/Risulta_";
    ...
    params[1] = log(randn(150,50));
	params[2] = log(randn(1,0.1));
    params[3] = log(randn(150,50));
	params[4] = log(randn(1,0.1));
    params[5] = log(randn(150,50));
    params[6] = log(randn(1,0.1));
    params[7] = log(randn(150,50));
    params[8] = log(randn(1,0.1));
	...
	for(int iter=1; iter<31; iter++){
	    ...
	    observed1 = solveODE(params, nReps, init1, N, Ob1, emul1A, emul2A, emul3A, dt, times1);
	    ...
	    for(int ii=1; ii<T; ii++){
	        ...
	        observed1 = solveODE(paramsNew, nReps, init1, N, Ob1, emul1A, emul2A, emul3A, dt, times1);
	        ...
	        if((paramsNew[0]<priorMax[0])&(paramsNew[1]<priorMax[1])&(paramsNew[2]<priorMax[2])&(paramsNew[3]<priorMax[3])&(paramsNew[4]<priorMax[4])&(paramsNew[5]<priorMax[5])&(paramsNew[6]<priorMax[6])&(paramsNew[7]<priorMax[7])&(paramsNew[8]<priorMax[8])&(negative<6))
    	    {
