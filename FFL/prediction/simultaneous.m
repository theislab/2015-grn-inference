...
% 10x decrease of Kd(+) and 5x decrease of Kd(-)
...
K_d_inhibition  = 200;
K_d_activationY = 400;
K_d_activation  = 400/K1;
n_inhibition    = 2;
n_activation    = 20;
synthS          = 100;
K_d_inhibitionS = 200/K2;
n_inhibitionS   = 10;
...
for j = 1:nRealiz
    ...
    for i = 1:T-1
        ...
        a(3,i+1) = a(3,i) + dt*(synthS*(1/(1+(K_d_activation/a(1,i))^n_activation)) + synthS*(1/(1+ (a(4,i)/K_d_inhibitionS)^n_inhibitionS)) - decay*a(3,i)) + sqrt(sigma)*randn;
        a(4,i+1) = a(4,i) + dt*(synthS*(1/(1+(K_d_activationY/a(1,i))^n_activation))  - decay*a(4,i)) + sqrt(sigma)*randn;
...
for i = 1:T-1
    ...
    a(3,i+1) = a(3,i) + dt*(synthS*(1/(1+(K_d_activation/a(1,i))^n_activation)) + synthS*(1/(1+ (a(4,i)/K_d_inhibitionS)^n_inhibitionS)) - decay*a(3,i));
    a(4,i+1) = a(4,i) + dt*(synthS*(1/(1+(K_d_activationY/a(1,i))^n_activation))  - decay*a(4,i));
...
decayZ           = paramZ(2);
K_d_activationZ  = paramZ(3)/K1;
n_activationZ    = paramZ(4);

K_d_inhibitionZ  = paramZ(5)/K2;
...
for i = 1:mT-1
    ...
    a(3,i+1) = a(3,i) + dt*(synthZ*(1/(1+(K_d_activationZ/a(1,i))^n_activationZ)) + synthZ*(1/(1+ (a(4,i)/K_d_inhibitionZ)^n_inhibitionZ)) - decayZ*a(3,i));
    a(4,i+1) = a(4,i) + dt*(synthY*(1/(1+(K_d_activationY/a(1,i))^n_activationY))  - decayY*a(4,i));
