...
std::vector<double> solveODE(double para[], int nReps, double init[], int N, int Ob, double emul1[][1], double emul2[][1], double dt, int times[])
{
	...
	for(int t=1; t<N; t++){
            ...
            k1 = exp(para[0])*exp(para[1])*((1/(1+pow(exp(para[2])/emul1[t-1][0],exp(para[3]))))+(1/(1+pow(exp(para[4])/emul2[t-1][0],-exp(para[5]))))) - exp(para[1])*x[t-1];
            k2 = exp(para[0])*exp(para[1])*((1/(1+pow(exp(para[2])/(0.5*(emul1[t-1][0]+emul1[t][0])),exp(para[3]))))+(1/(1+pow(exp(para[4])/(0.5*(emul2[t-1][0]+emul2[t][0])),-exp(para[5]))))) - exp(para[1])*(x[t-1]+0.5*k1);
            k3 = exp(para[0])*exp(para[1])*((1/(1+pow(exp(para[2])/(0.5*(emul1[t-1][0]+emul1[t][0])),exp(para[3]))))+(1/(1+pow(exp(para[4])/(0.5*(emul2[t-1][0]+emul2[t][0])),-exp(para[5]))))) - exp(para[1])*(x[t-1]+0.5*k2);
            k4 = exp(para[0])*exp(para[1])*((1/(1+pow(exp(para[2])/emul1[t][0],exp(para[3]))))+(1/(1+pow(exp(para[4])/emul2[t][0],-exp(para[5]))))) - exp(para[1])*(x[t-1]+k3);
            
            x[t] = x[t-1] + (k1+2*k2+2*k3+k4)/6;
		                
	}
	x[N] = x[N-1] + dt*(exp(para[0])*exp(para[1])*((1/(1+pow(exp(para[2])/emul1[N-1][0],exp(para[3]))))+(1/(1+pow(exp(para[4])/emul2[N-1][0],-exp(para[5]))))) - exp(para[1])*x[N-1]);
...
double addPrior(double sample)
{
	double like;
	like = -0.5 * pow(((log(0.25) - sample)/0.001),2);
	return like;
}
